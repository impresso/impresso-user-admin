...
def is_task_stopped(task, job, progress=None, extra={}):
    ...
    update_job_progress(
        task=task, job=job,
        progress=progress if progress else job.get_progress(),
        taskstate=TASKSTATE_STOPPED, extra=extra)
    return True
...
def test(self, user_id):
    ...
    update_job_progress(
        task=self, job=job,
        taskstate=TASKSTATE_INIT, progress=0.0)
...
def export_query_as_csv_progress(
    self, job_id, query, search_query_id, query_hash='',
    skip=0, limit=100
):
    # get the job so that we can update its status
    job = Job.objects.get(pk=job_id)
...
def store_collection_progress(
    ...
    page, loops, progress, total, allowed = sync_query_to_collection(
        collection_id=collection_id, query=query, content_type=content_type,
        skip=skip, limit=limit, method=method, logger=logger)
    extra.update({
        'total': total,
        'allowed': allowed,
    })
...
def store_collection(
    self, collection_id, items_ids=[], method=METHOD_ADD_TO_INDEX,
    content_type='A'
):
    '''
    ...
    logger.info(
        f'Collection(pk:{collection.pk}) '
        f'items={items_ids} method={method}')
    # start update chain
    store_collection_progress.delay(
        job_id=job.pk,
...
def add_to_collection_from_query_progress(
    self, query, fq, job_id, collection_id, content_type,
    skip=0, limit=100, prev_progress=0.0,
    ...
    page, loops, progress, total, allowed = sync_query_to_collection(
        collection_id=collection_id, query=query, content_type=content_type,
        skip=skip, limit=limit, logger=logger)
    extra = {
        'collection': get_collection_as_obj(collection),
        'total': total,
        'allowed': allowed,
    ...
    if progress < 1.0:
        ...
        add_to_collection_from_query_progress.delay(
            query=query, fq=fq, job_id=job_id, collection_id=collection_id,
            content_type=content_type,
            skip=skip+limit,
            limit=limit,
            serialized_query=serialized_query
...
def remove_collection(self, collection_id, user_id):
    ...
    try:
        ...
        collection_to_delete = (
            collection.status == Collection.DELETED and
            collection.creator.pk == user_id
        )
    ...
    if not collection_to_delete:
        logger.info(f'Cannot delete collection pk={collection_id},not to DEL!')
    ...
    update_job_progress(
        task=self, job=job, taskstate=TASKSTATE_INIT, progress=0.0, extra={
            'collection': collection_seralized
        }
    )
...
def remove_collection_progress(
    self, job_id, collection_id, user_id, skip=0, limit=100, progress=0.0,
):
    job = Job.objects.get(pk=job_id)
    ...
    if progress < 1.0:
        ...
        logger.info(
            f'remove_collection_progress completed page={page} loops={loops}'
        )
        try:
...
def update_collections_in_tr_passages(
    self, collection_prefix, user_id=None
):
...
def remove_collection_in_tr(
    ...
    # save current job!
    job = Job.objects.create(
        type=Job.REMOVE_COLLECTIONS_FROM_SOLR_TR,
        creator_id=user_id,
        status=Job.RUN
    )
    if not collection_to_delete:
...
def remove_collection_in_tr_progress(
    self, collection_id, job_id, skip=0, limit=100
):
    # get the job so that we can update its status
    job = Job.objects.get(pk=job_id)
...
def after_user_registered(self, user_id):
    logger.info(
        f'user({user_id}) just registered'
    )
